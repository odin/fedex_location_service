...
module FedexLocationService
  class Locations
    def self.call(response)
      status = response[:search_locations_reply][:highest_severity]

      if status == 'SUCCESS'
        locations        = response[:search_locations_reply][:address_to_location_relationships][:distance_and_location_details]
        location_details = []
